...
unsigned int ShoppingList::size() const
{
    return (listSize);
}

std::string const& ShoppingList::item(unsigned int index) const
{
    return itemList[index].item;
}

unsigned int ShoppingList::count(unsigned int index) const
{
...
void ShoppingList::add(std::string const& item, unsigned int count) // version von Tobias Glasmachers
{
	for (unsigned int i=0; i<listSize; i++)
	{
	...
	auto newarray = new Entry[listSize + 1];
